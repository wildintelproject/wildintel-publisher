    ...
    <div className="mx-auto px-4 py-4" style={{ maxWidth: 700 }}>
      ...
      {step === 3 && publishStarted && !allConfigured && !executing && !executionDone && (
        <div>
          ...
          {(() => {
            // camtrapdp-remote.zip (the only archive GBIF can actually crawl)
            // is only ever generated by HFH's own Mirror mode (see
            // hfh.upload_to_huggingface) — Link mode never creates it, so
            // there's nothing deterministic to point GBIF at in that case.
            // toggleRepo always forces HFH before GBIF whenever both are
            // selected, so repoConfigs.hfh is already collected by the time
            // GBIF's own form renders.
            const hfhWillProduceRemoteZip = publishOrder.includes('hfh') && repoConfigs.hfh?.mirrorImages === true
              ...
              <GBIFPublishForm
                ...
                suggestedArchiveUrl={
                  hfhWillProduceRemoteZip
                    ...
                    // No HFH producing a remote zip in this run (either not
                    // selected, or publishing in Link mode) — if the source
                    // itself was a public archive URL (see
                    // CamtrapDPArchiveForm), that URL is already confirmed
                    // public and a valid Camtrap DP (the fetch step
                    // validates it the same way GBIF itself would need it to
                    // be), so it's directly reusable here as-is.
